...
class NavInputPill extends ConsumerStatefulWidget {
  ...
  const NavInputPill({super.key, required this.onSend});
...
class _NavInputPillState extends ConsumerState<NavInputPill>
  ...
  void _onTextChanged() {
    if (_currentState == NavInputState.initial ||
        _currentState == NavInputState.typing) {
  ...
  void _handleFirstSubmit() async {
    ...
    try {
      ...
      final historyMessages = chatHistory
          .map(
            (msg) => {
      ...
      if (mounted) {
        ...
        if (_wasVoiceInput && _currentState == NavInputState.generating) {
          String spokenText = _aiResponse.replaceAll(
            RegExp(r'```json[\s\S]*?```'),
            '',
          );
      ...
      if (mounted) {
        _aiResponse =
            "I apologize, but I encountered an error. Let's head to the main chat to sort this out.";
        setState(() => _currentState = NavInputState.streaming);
        if (_wasVoiceInput)
          _aiService.speak("I apologize, but I encountered an error.");
      }
    }
  }
  ...
  void _handleSecondSubmit() {
    ...
    ref
        ...
        .sendMessage(text, isVoice: _wasVoiceInput);
  ...
  void _toggleListening() {
    ...
    if (speechState.isListening) {
      ...
      FocusScope.of(
        context,
      ).unfocus(); // Close keyboard so it doesn't interrupt STT
      final isFirstPhase =
          _currentState == NavInputState.initial ||
          _currentState == NavInputState.typing;
      final existingText = isFirstPhase
          ? _controller.text
        ...
        onResultCallback: (text) {
          ...
          if (text.isNotEmpty) {
            ...
            if (isFirstPhase) {
              ...
              _secondController.selection = TextSelection.fromPosition(
                TextPosition(offset: _secondController.text.length),
              );
            }
          }
        },
                  ...
                  AnimatedSize(
                    ...
                    child: Container(
                      decoration: BoxDecoration(
                        color: const Color(
                          0xFFE8F0FE,
                        ), // Google blue-ish pill background
                      ...
                      child: Row(
                        ...
                        children: [
                          if (!isTyping &&
                              !ref.watch(speechProvider).isListening) ...[
                            GestureDetector(
                              onTap: () {
                          ...
                          GestureDetector(
                            ...
                            child: Container(
                              ...
                              child: Icon(
                                (_currentState == NavInputState.streaming ||
                                        _currentState ==
                                            NavInputState.generating ||
