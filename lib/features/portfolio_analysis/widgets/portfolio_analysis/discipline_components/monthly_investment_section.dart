...
class MonthlyInvestmentSection extends StatefulWidget {
  ...
  @override
  State<MonthlyInvestmentSection> createState() => _MonthlyInvestmentSectionState();
}

class _MonthlyInvestmentSectionState extends State<MonthlyInvestmentSection> with SingleTickerProviderStateMixin {
        ...
        children: [
          ...
          AspectRatio(
            ...
            child: GestureDetector(
              ...
              onHorizontalDragDown: (details) => _handleTouch(details.localPosition, context.size!.width),
              onHorizontalDragUpdate: (details) => _handleTouch(details.localPosition, context.size!.width),
              onHorizontalDragEnd: (_) => _handleTouchEnd(),
              onHorizontalDragCancel: () => _handleTouchEnd(),
              onTapDown: (details) => _handleTouch(details.localPosition, context.size!.width),
                ...
                RichText(
                  text: const TextSpan(
                    ...
                    children: [
                      ...
                      TextSpan(
                        text: 'your investing rhythm has some breaks. keeping it steady will grow your money faster.',
                      ),
                    ],
                  ),
                ),
...
class _MonthlyNetInvestmentPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i <= numCols; i++) {
      ...
      canvas.drawLine(Offset(x, 20 * scale), Offset(x, size.height - (20 * scale)), gridPaint);
    }
    ...
    final zeroPaint = Paint()
      ...
      ..style = PaintingStyle.stroke
      ..strokeWidth = 1 * scale;
    _drawDashedLine(canvas, Offset(paddingX, y0), Offset(size.width - paddingX, y0), zeroPaint, scale);
    ...
    void drawLabel(String text, double y) {
      textPainter.text = TextSpan(
        text: text,
        style: TextStyle(fontFamily: 'DMSans', fontSize: 10 * scale, color: const Color(0xFF94A3B8)),
      );
      textPainter.layout();
    ...
    final labels = ["AUG'25", "SEP'25", "OCT'25", "NOV'25", "DEC'25", "JAN'26", "FEB'26", "MAR'26", "AUG'26"];
    final netValues = ["₹10K", "₹50K", "₹10K", "₹5K", "₹-30K", "₹0", "₹0", "₹0", "₹0"];
    final buyValues = ["₹20K", "₹50K", "₹20K", "₹15K", "₹0", "₹0", "₹0", "₹0", "₹0"];
    final sellValues = ["₹10K", "₹0", "₹10K", "₹10K", "₹30K", "₹0", "₹0", "₹0", "₹0"];
    ...
    if (progress > 0.8 && hoverIndex == 8 && !isExpanded) {
      final avgPillRect = RRect.fromRectAndRadius(
        Rect.fromCenter(center: Offset(paddingX + 4 * stepX, y0 - (2 * scale)), width: 70 * scale, height: 20 * scale),
    ...
    if (progress > 0.9) {
      _drawAxisTag(canvas, 'AUG\'25', Offset(paddingX, size.height - (12 * scale)), size, scale);
      _drawAxisTag(canvas, 'AUG\'26', Offset(paddingX + 8 * stepX, size.height - (12 * scale)), size, scale);
      ...
      // Vertical line
      canvas.drawLine(Offset(pt.dx, tooltipBaseY), Offset(pt.dx, size.height - (20 * scale)), Paint()..color = const Color(0xFF94A3B8)..strokeWidth = 1 * scale);
      
      // Dot on graph
      canvas.drawCircle(pt, 5 * scale, Paint()..color = Colors.white);
      canvas.drawCircle(pt, 5 * scale, Paint()..color = Colors.black..style = PaintingStyle.stroke..strokeWidth = 2 * scale);
      ...
      if (isExpanded) {
        ...
        _drawValueTooltip(canvas, labels[hoverIndex], '${netValues[hoverIndex]} >', tooltipPoint, size, scale);
      }
    }
  }

  void _drawAxisTag(Canvas canvas, String text, Offset center, Size size, double scale) {
    final textPainter = TextPainter(
      text: TextSpan(text: text, style: TextStyle(fontFamily: 'DMSans', fontSize: 9 * scale, fontWeight: FontWeight.w700, color: Colors.black)),
    ...
    final rect = Rect.fromLTWH(left, center.dy - boxHeight / 2, boxWidth, boxHeight);
    
    canvas.drawRRect(RRect.fromRectAndRadius(rect, Radius.circular(4 * scale)), Paint()..color = Colors.white);
    canvas.drawRRect(RRect.fromRectAndRadius(rect, Radius.circular(4 * scale)), Paint()..color = Colors.black..style = PaintingStyle.stroke..strokeWidth = 1.5 * scale);
    ...
    canvas.drawPath(pPath, Paint()..color = Colors.black..style = PaintingStyle.stroke..strokeWidth = 1.5 * scale);
  ...
  void _drawValueTooltip(Canvas canvas, String title, String value, Offset point, Size size, double scale) {
    final textPainter = TextPainter(textDirection: TextDirection.ltr);
    textPainter.text = TextSpan(text: value, style: TextStyle(fontFamily: 'DMSans', fontSize: 10 * scale, fontWeight: FontWeight.w700, color: Colors.black));
    textPainter.layout();
    final valueWidth = textPainter.width;
    
    textPainter.text = TextSpan(text: title, style: TextStyle(fontFamily: 'DMSans', fontSize: 9 * scale, fontWeight: FontWeight.w600, color: const Color(0xFF64748B)));
    textPainter.layout();
    ...
    canvas.drawRRect(rrect, Paint()..color = Colors.black..style = PaintingStyle.stroke..strokeWidth = 1.5 * scale);
    
    // Title
    textPainter.text = TextSpan(text: title, style: TextStyle(fontFamily: 'DMSans', fontSize: 9 * scale, fontWeight: FontWeight.w600, color: const Color(0xFF64748B)));
    textPainter.layout();
    textPainter.paint(canvas, Offset(rect.left + (8 * scale), rect.center.dy - textPainter.height / 2));

    // Value
    textPainter.text = TextSpan(text: value, style: TextStyle(fontFamily: 'DMSans', fontSize: 10 * scale, fontWeight: FontWeight.w700, color: Colors.black));
    textPainter.layout();
    textPainter.paint(canvas, Offset(rect.right - textPainter.width - (8 * scale), rect.center.dy - textPainter.height / 2));
  ...
  void _drawExpandedTooltip(Canvas canvas, String title, String buy, String sell, String net, Offset point, Size size, double scale) {
    ...
    final shadedRect = Rect.fromLTWH(left, top + boxHeight - (28 * scale), boxWidth, 28 * scale);
    ...
    final pPath = Path()
      ...
      ..lineTo(point.dx + (6 * scale), rect.bottom);
    canvas.drawPath(pPath, Paint()..color = Colors.black..style = PaintingStyle.stroke..strokeWidth = 1.5 * scale);
    canvas.drawPath(pPath, Paint()..color = const Color(0xFFF1F5F9)..style = PaintingStyle.fill);
    ...
    void drawRow(String label, String value, double y, bool boldLabel) {
      textPainter.text = TextSpan(
        text: label,
        style: TextStyle(fontFamily: 'DMSans', fontSize: 10 * scale, fontWeight: boldLabel ? FontWeight.w700 : FontWeight.w600, color: const Color(0xFF0F172A)),
      );
      textPainter.layout();
      ...
      textPainter.layout();
      textPainter.paint(canvas, Offset(left + boxWidth - (12 * scale) - textPainter.width, y));
    }
    ...
    canvas.drawCircle(Offset(left + (16 * scale), top + boxHeight - (14 * scale)), 3 * scale, Paint()..color = Colors.white);
    canvas.drawCircle(Offset(left + (16 * scale), top + boxHeight - (14 * scale)), 3 * scale, Paint()..color = Colors.black..style = PaintingStyle.stroke..strokeWidth = 2 * scale);
    ...
    textPainter.layout();
    textPainter.paint(canvas, Offset(left + (26 * scale), top + boxHeight - (20 * scale)));
    ...
    textPainter.layout();
    textPainter.paint(canvas, Offset(left + boxWidth - (12 * scale) - textPainter.width, top + boxHeight - (20 * scale)));
  }

  void _drawDashedLine(Canvas canvas, Offset p1, Offset p2, Paint paint, double scale) {
  ...
  bool shouldRepaint(covariant _MonthlyNetInvestmentPainter oldDelegate) {
    return oldDelegate.progress != progress || oldDelegate.hoverIndex != hoverIndex || oldDelegate.isExpanded != isExpanded;
  }
}
